...
bot.start(async (ctx) => {
    ...
    // В режиме разработки отправляем картинку с сообщением
    if (process.env.NODE_ENV !== 'production') {
        const message = await ctx.replyWithPhoto({ source: './public/start_1.png' }, {
            caption: `Эй, йоу! Добро пожаловать в Total Lookas! Мы создаем классный мерч и можем сделать тебе!\n\n` +
                `Всё просто:\n` +
                `1. Нажми «НАЧАТЬ»\n` +
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: '� НАЧАТЬ', callback_data: 'start_catalog' }],
    ...
    // В продакшене отправляем картинку с веб-приложением
    const message = await ctx.replyWithPhoto({ source: './public/start_1.png' }, {
        caption: `Эй, йоу! Добро пожаловать в Total Lookas! Мы создаем классный мерч и можем сделать тебе!\n\n` +
            `Всё просто:\n` +
            `1. Нажми «НАЧАТЬ»\n` +
        ...
        reply_markup: {
            inline_keyboard: [
                [{ text: '� НАЧАТЬ', web_app: { url: webAppUrl } }],
                [{ text: '📞 Связаться с нами', callback_data: 'contact' }],
                [{ text: 'ℹ️ О компании', callback_data: 'about' }]
            ]
        }
...
bot.action('start_catalog', async (ctx) => {
    ...
    // Отправляем новое сообщение с ссылкой на каталог
    const message = await ctx.reply(`🚀 **Каталог Total Lookas**\n\n` +
        `Откройте каталог в браузере для выбора товаров:\n` +
        `${webAppUrl}\n\n` +
        `В каталоге вы сможете:\n` +
...
bot.action('contact', async (ctx) => {
    ...
    const message = await ctx.reply(`📞 **Связаться с нами**\n\n` +
...
bot.action('about', async (ctx) => {
    ...
    const message = await ctx.reply(`ℹ️ **О компании Total Lookas**\n\n` +
        `🎨 **Кто мы?**\n` +
...
bot.action('catalog', async (ctx) => {
    ...
    const message = await ctx.reply(`🛍 **Каталог товаров Total Lookas**\n\n` +
        `👕 **Одежда:**\n` +
        `• Футболки и поло\n` +
        `• Худи и свитшоты\n` +
        `• Zip-худи\n` +
...
bot.action('back_to_main', async (ctx) => {
    ...
    const message = await ctx.reply(`👋 **Главное меню Total Lookas**\n\n` +
...
bot.command('id', async (ctx) => {
    if (process.env.NODE_ENV === 'development') {
        ...
        ctx.reply(`🆔 Ваш Telegram ID: \`${userId}\`\n\n` +
            `Этот ID можно использовать для тестирования отправки КП в режиме разработки.`, { parse_mode: 'Markdown' });
    }
    else {
...
bot.on('text', async (ctx) => {
    ...
    if (process.env.NODE_ENV !== 'production') {
        ...
        const message = await ctx.reply(`Привет! 😊 Для просмотра каталога откройте ${webAppUrl} в браузере.\n\n` +
...
bot.on('web_app_data', async (ctx) => {
    ...
    try {
        ...
        if (data.type === 'commercial_proposal') {
            ...
            // Отправляем пользователю сообщение о начале генерации КП
            const processingMessage = await ctx.reply('🔄 Генерируем коммерческое предложение...\n' +
                'Это может занять несколько секунд.');
            if (ctx.chat)
                saveMessageId(ctx.chat.id, processingMessage.message_id);
            // Формируем сообщение для администратора
            const adminMessage = `🔔 *Новое коммерческое предложение!*\n\n` +
                `👤 *Клиент:* ${orderData.customerName}\n` +
                `🏢 *Компания:* ${orderData.customerCompany || 'Не указана'}\n` +
                `🔢 *ИНН:* ${orderData.customerInn || 'Не указан'}\n` +
                ...
                `📎 КП будет отправлено клиенту автоматически.`;
            ...
            try {
                // Пытаемся отправить уведомление администратору
                if (process.env.ADMIN_CHAT_ID) {
                    await bot.telegram.sendMessage(process.env.ADMIN_CHAT_ID, adminMessage, { parse_mode: 'Markdown' });
                }
                // Отвечаем пользователю
                const message = await ctx.reply('✅ Ваше коммерческое предложение готово и будет отправлено через несколько секунд!\n\n' +
                    '📋 После получения КП вы можете:\n' +
                    '1. Проверить все детали заказа\n' +
                    '2. Показать КП вашему руководителю\n' +
                    '3. Связаться с нами для уточнения деталей\n\n' +
                    '💡 Если у вас возникнут вопросы, просто напишите их в этот чат.');
                if (ctx.chat)
                    saveMessageId(ctx.chat.id, message.message_id);
                // Добавляем паузу перед отправкой следующего сообщения
                await new Promise(resolve => setTimeout(resolve, 1000));
            }
            catch (error) {
                console.error('Ошибка при обработке КП:', error);
                const errorMessage = await ctx.reply('❌ Произошла ошибка при обработке коммерческого предложения.\n' +
                    'Пожалуйста, попробуйте еще раз или свяжитесь с нашим менеджером.');
                if (ctx.chat)
                    saveMessageId(ctx.chat.id, errorMessage.message_id);
            }
        }
        else {

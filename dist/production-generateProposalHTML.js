...
exports.generateProposalHTML = generateProposalHTML;
...
function formatItemOptions(options, showDetails) {
    if (!showDetails || !options)
        return '';
    return Object.entries(options).map(([category, values]) => {
        const translatedCategory = CATEGORY_TRANSLATIONS[category] || category;
        const translatedValues = values.map(value => value.startsWith('cme15m') ? 'Индивидуально' : value).join(', ');
        return `
